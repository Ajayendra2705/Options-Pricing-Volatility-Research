...
**Next — Day 33:** RV/HAR/signal on SPY (`realized_vol`/`har`/`signal` still hardcode `PROCESSED_DIR` — same seam treatment), `config/spy_phase2.yaml` per the pre-registration, then **walk-forward folds across the 147 sessions**. Open questions for the folds: whether to exclude the 2 calendar-inverted dates (they are legitimate data, but their ATM marks are lifted off the quotes); and note the per-date claimed k domain gets narrow when the front slice is thinly quoted (narrowest: k ∈ [−0.014, +0.011]), which is fine for an ATM signal but bounds any wing work.
